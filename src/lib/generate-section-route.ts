...
import type { DetailSection, ProductInfo, SectionKind, Tone } from "@/src/types";
...
type ValidationResult =
  | { ok: true; data: GenerateSectionRequest }
...
export async function POST(request: Request) {
  ...
  const { product, sectionKind, generationMode } = validation.data;
  const generatedAt = new Date().toISOString();
  const section = createSectionShell(sectionKind);
  const content = generateDummySectionCopy(product, section, {
